...
def refresh(segmentid):
    ...
    tpos=str(segmentid)+"/"+str(contmax)
    tposition.set(tpos)
    if status=="done":
...
def go_to():
    ...
    position=int(segnum)
    if position>0 and position<=contmax:
        lenT2=len(T2.get("1.0",END))
        info=("OUT", lastsegment, datetime.datetime.now(),lenT2) 
        cur.execute("INSERT INTO actions (tipus, segment_id, datetime, len_posted) VALUES (?,?,?,?)",info)
        refresh(position)
...
def next(_event=None):
    ...
    cur.execute("INSERT INTO actions (tipus, segment_id, datetime, len_posted) VALUES (?,?,?,?)",info)
    lastsegment+=1
    if lastsegment>contmax: lastsegment=1
    refresh(lastsegment)
    lenT2=len(T2.get("1.0",END))
    info=("IN", lastsegment, datetime.datetime.now(),lenT2) 
    cur.execute("INSERT INTO actions (tipus, segment_id, datetime, len_posted) VALUES (?,?,?,?)",info)
...
def previous(_event=None):
    ...
    cur.execute("INSERT INTO actions (tipus, segment_id, datetime, len_posted) VALUES (?,?,?,?)",info)
    lastsegment-=1
    if lastsegment==0: lastsegment=contmax
    refresh(lastsegment)
    lenT2=len(T2.get("1.0",END))
    info=("IN", lastsegment, datetime.datetime.now(),lenT2) 
    cur.execute("INSERT INTO actions (tipus, segment_id, datetime, len_posted) VALUES (?,?,?,?)",info)
...
cur.execute('SELECT datetime FROM actions')
dts=cur.fetchall()
...
cur.execute("SELECT MAX(segment_id) FROM actions;")
...
if lastid==None:
    ...
    cur.execute('SELECT segment_id FROM actions WHERE segment_id=?', (lastid,))
    lastsegment=cur.fetchone()[0]
